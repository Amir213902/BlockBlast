...
export const createBlocks = (grid = createGrid()) => {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    const blocks = Array.from({ length: BLOCK_COUNT }, (_, index) => createBlock(index));
  ...
  return Array.from({ length: BLOCK_COUNT }, (_, index) => createBlock(index, index === 0 ? playableShape : randomShape()));
};
